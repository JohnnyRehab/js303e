(function(){ // TODO: Fix sluggishness in wrapper
	...
	$(function(){
		...
		function applyDelayTime() {
			var steps = synth.delay_steps || 3;
			var len = Math.round(synth.steplength * steps);
			var max = synth.delaybuffer.length - 1;
			if (len < 64) { len = 64; }
			if (len > max) { len = max; }
			synth.delay_length = len;
			if (synth.delaypos > synth.delay_length) {
				synth.delaypos = 0;
			}
		}
		...
		synth.delay_steps = 3;
		applyDelayTime();
		$(".step-choice[data-steps='3']").addClass("on").attr("aria-pressed", "true");

		$("#delay-send").on("input change", function(){
			synth.delay_send = Number(this.value);
		});
		$("#delay-feedback").on("input change", function(){
			synth.delay_feedback = Number(this.value);
		});
		$(".step-choice").click(function(){
			$(".step-choice").removeClass("on").attr("aria-pressed", "false");
			$(this).addClass("on").attr("aria-pressed", "true");
			synth.delay_steps = Number($(this).attr("data-steps"));
			applyDelayTime();
		});
		...
		$("#run").click(function(e){
			...
			$(this).toggleClass("running", synth.running);
			$(this).text(synth.running ? "Stop" : "Run");
		});
		...
		setTimeout(function() {
			if (typeof wavetable === "undefined" || !wavetable) {
				if (typeof genwavetable === "function") {
					genwavetable();
				}
			}
		}, 2500);
